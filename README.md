...
2. リポジトリ: `https://github.com/solidlime/Daikin-MCZ70-HomeAssist`、カテゴリ: **Integration**
...
| 書き込みが `ret=LOCK NG` で失敗する | ダイキンサーバー側の一時的な端末ロック（トークンは有効）。実機の電源をコンセントから抜いて 30 秒待ち、再度差し込むと解消します（実機検証済み。ファームウェア自動更新直後にも発生しました） |
...
- **空気質センサー（PM2.5 / ほこり / におい）**: この機種のファームウェア（3.8.0 / 3.15.0 で確認）では、ローカル API の現在値が**常に 0** を返します。デバイス側の日次/週次履歴（`get_day_snsr_count` / `get_week_snsr_count`）には実値が記録されており、Daikin アプリはクラウド経由で現在値を表示しています（センサー自体は正常）。クラウド側の `get_sensor_info` も 404 で取得できません（クラウド API は制御専用のため、0 固定が仕様です）。
