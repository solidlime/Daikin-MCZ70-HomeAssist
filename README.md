...
| セレクト | 運転モード（空気清浄 / 除湿空気清浄 / 加湿空気清浄）、風量（自動 / しずか / 弱 / 標準 / 高 / ターボ）、湿度モード（低め / 標準 / 高め / 連続・設定は次回加湿運転時に反映）、LED表示（明 / 暗め / 消灯） |
| センサー | 室温 / 室内湿度 / PM2.5 / ほこり / におい |
| バイナリセンサー | 給水が必要 / 加湿タンク未装着 |
...
- **タンクフラグ**（実機検証済み）: 加湿タンクのフラグは **1 = 正常（装着・空） / 0 = 警告（未装着・満水）** です。加湿タンクが「空」の時に 0 になるかは未検証（加湿シーズンに確認予定）。除湿タンクのフラグは挙動が不明瞭なため、エンティティとしては現在提供していません（必要になったら再追加予定）。
